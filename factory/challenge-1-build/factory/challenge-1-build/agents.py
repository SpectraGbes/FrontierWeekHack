...
MODEL_DEPLOYMENT_NAME = os.getenv("MODEL_DEPLOYMENT_NAME", "gpt-4.1")  # ✅ switched default model
...
def check_thresholds(machine_id: str) -> str:
    ...
    machine = next((m for m in data["machines"]
                    if m["machine_id"] == machine_id or m["name"] == machine_id), None)
...
CHECK_THRESHOLDS_TOOL = FunctionTool(
    ...
    description="Check if a machine's sensor readings are within normal operating thresholds.",
    parameters={
        "type": "object",
        "properties": {
            "machine_id": {
...
class AnomalyDetectionAgent:
    ...
    def create(self):
        ...
        self.client = AIProjectClient.from_connection_string(PROJECT_CONNECTION_STRING)  # ✅ fixed init
        self.openai = self.client.get_openai_client()

        system_prompt = """
        You are an industrial sensor anomaly detection expert for TireForge Industries.
        Use the check_thresholds tool for each machine.
        Report machine ID, status, and anomalies concisely.
        """
    ...
    def run(self, input_text: str) -> str:
        ...
        print(f"⏱️ Anomaly detection response time: {duration:.2f}s")
...
class FaultDiagnosisAgent:
    ...
    def create(self):
        ...
        Identify root cause, recommend maintenance, and estimate urgency.
        """
    ...
    def run(self, input_text: str) -> str:
        ...
        duration = time.time() - start
        print(f"⏱️ Fault diagnosis response time: {duration:.2f}s")
...
def main():
    ...
    machine_ids = [m["machine_id"] for m in machine_batch]
    anomaly_result = anomaly_agent.run(
        f"Process batch machines: {json.dumps(machine_ids)}\n{json.dumps(machine_batch, indent=2)}"
    )
    ...
    critical_batch = [m for m in machine_batch if m["status"] in {"critical", "warning"}]
    diagnosis_result = diagnosis_agent.run(
        f"Diagnose critical machines:\n{json.dumps(critical_batch, indent=2)}"
    )
